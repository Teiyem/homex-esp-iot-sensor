...
/**
 * It waits for the pin to go high or low, and returns the time it took to do so
 * in pointer 'duration' if it is not NULL.
 * @param pin The GPIO pin number that the sensor is connected to.
 * @param timeout The time to wait for the pin to change state.
 * @param expected_pin_state The expected state of the pin.
 * @param duration The time in microseconds that the pin was in the expected state
 * @return false if timeout is reached and pin doesn't go to a requested state
 */
...
/**
 * It pulls the data pin low for a short period of time, then waits for the sensor to pull the data pin
 * high for a short period of time, then waits for the sensor to pull the data pin low for a short
 * period of time, then reads in 40 bits of data
 * The function call should be protected from task switching.
 * @param sensor_type DHT11, DHT22 or Itead SI7021
 * @param pin The GPIO pin number that the sensor is connected to.
 * @param bits The array of bits that will be returned.
 * 
...
/**
 * Takes the two bytes of data from the DHT sensor and converts it to a signed integer
 * @param sensor_type DHT11, DHT22 or Itead SI7021
 * @param msb Most significant byte of the data
 * @param lsb The least significant byte of the data.
 * @return The temperature and humidity as a single value.
 */
...
/**
  * Read data from sensor on specified pin. dht_init must be called before reading it.
  * Humidity and temperature is returned as integers.
  * For example: humidity=625 is 62.5 %
  *                      temperature=244 is 24.4 degrees Celsius
  * @param sensor_type DHT11, DHT22 or Itead SI7021
  * @param pin The GPIO pin number that the DHT sensor is connected to.
  * @param humidity The output humidity
  * @param temperature The output temperature
  * @return
  *    - ESP_OK Success
  *    - ESP_FAIL Init error
  */
...
/**
 * Float version of dht_read_data Reads the data from the DHT sensor and converts it to float.
 * dht_init must be called before reading it. Humidity and temperature is returned as float.
 * For example: humidity=62.5 is 62.5 % temperature=24.4 is 24.4 degrees Celsius
 * @param sensor_type DHT11, DHT22 or Itead SI7021
 * @param pin The GPIO pin number that the DHT sensor is connected to.
 * @param humidity The output humidity
 * @param temperature The output temperature
...
/**
 * Initialize the GPIO pin as an open-drain output with a pull-up resistor
 * @param pin The GPIO pin number that the DHT sensor is connected to.
 * @param pull_up If true, the GPIO will be configured with a pull-up resistor. If false, the GPIO will
 * be configured with a pull-down resistor.
 * @return
 *    - ESP_OK Success
